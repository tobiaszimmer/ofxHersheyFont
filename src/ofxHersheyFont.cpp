...
//--------------------------------------------------------------
ofPath ofxHersheyFont::getPathFromAscii(int asciiValue, float scale) {
    ...
    for (int i = 4; i <= simplex[asciiValue - 32][0] * 2; i += 2)
    {
        int x = simplex[asciiValue - 32][i] * scale;;
        int y = simplex[asciiValue - 32][i + 1] * scale;;
        ...
        //and move to next point
        if (x == -1) {
            chPath.moveTo(simplex[asciiValue - 32][i + 2]* scale, simplex[asciiValue - 32][i + 3]* scale);
            i += 2;
        }
    }
...
ofPath ofxHersheyFont::getPathFromXML(ofXml xmlElement, float scale){
...
//    chPath.moveTo(simplex[asciiValue - 32][2], simplex[asciiValue - 32][3]);
    ...
    for(int i=0; i<splitString.size(); i+=3){
        if(splitString[i] == "M"){
            chPath.moveTo(ofToFloat(splitString[i+1])* scale, ofToFloat(splitString[i+2])* scale);
        }else if(splitString[i] == "L"){
            chPath.lineTo(ofToFloat(splitString[i+1])* scale, ofToFloat(splitString[i+2])* scale);
        }
    }
